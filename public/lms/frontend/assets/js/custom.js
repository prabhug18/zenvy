(function ($) {
    ...
    function couponInformation(data) {
        ...
        // detect currency symbol from existing subtotal text (fallback to ₹)
        let subText = $("#subTotal").text().trim();
        let currencyMatch = subText.match(/^[^0-9\-\s]+/);
        let currencySymbol = currencyMatch ? currencyMatch[0] : '₹';
        ...
        $("#discount-area").html(`
                ...
                <td class="px-1 py-4 text-right" id="discount-area">
                    <div class="text-heading/70 font-semibold leading-none">${currencySymbol}${coupon_amount}</div>
                </td>

           `);

        let grand = (parseFloat(total_amount) - parseFloat(coupon_amount)).toFixed(2);

        $("#total").html(`${currencySymbol}${total_amount}`);
        $("#subTotal").html(`${currencySymbol}${total_amount}`);
        $("#grand_total").html(`${currencySymbol}${grand}`);
