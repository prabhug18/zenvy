        ...
        @switch($enrolled->status)
            ...
            @case('complete')
                <div class="flex items-center gap-2">
                    ...
                    @php
                        $userCertificate = Modules\LMS\Models\Certificate\UserCertificate::where('user_id', authCheck()->id)
                            ->whereIn('quiz_id', function($query) use ($enrolled) {
                                $query->select('id')->from('user_course_exams')->where('course_id', $enrolled->course_id);
                            })->first();
                    @endphp
                    @if ($userCertificate)
                        <a href="{{ route('student.certificate.download', $userCertificate->id) }}" target="_blank"
                            class="badge b-solid badge-success-solid capitalize" title="{{ translate('Certificate') }}">
