...
<form action="{{ route($topicRoute) }}" class="flex flex-col form pb-4" enctype="multipart/form-data">
    ...
    @if ($type == 'reading')
        ...
        <div class="mt-4">
            <label class="form-label">{{ translate('Attach PDF') }}</label>
            <div class="border border-input-border rounded-md px-2 py-1.5">
                <input type="file" name="pdf_file" class="w-full" accept=".pdf">
            </div>
            <span class="text-danger error-text pdf_file_err"></span>
            @if (!empty($topic) && $topic->topicable->pdf_file)
                <div class="mt-2">
                    <a href="{{ asset('storage/lms/courses/topics/readings/' . $topic->topicable->pdf_file) }}" target="_blank" class="text-primary-500 underline">
                        {{ translate('View Current PDF') }}
                    </a>
                </div>
            @endif
        </div>
