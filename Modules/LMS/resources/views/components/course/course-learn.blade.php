...
@if ($type == 'video')
    ...
    @endif
@elseif($type == 'reading')
    <div class="p-5 md:p-8 xl:p-10 relative bg-primary-50 rounded-xl">
        <div class="w-full">
            <h5 class="area-title text-xl font-bold mb-4" style="word-wrap: break-word; overflow-wrap: break-word;">{{ $data->title }}</h5>
            <div class="area-description mt-5" style="word-wrap: break-word; overflow-wrap: break-word; white-space: normal; max-width: 100%;">
                {!! clean($data->description) !!}
            </div>

            @if ($data->pdf_file && fileExists('lms/courses/topics/readings/', $data->pdf_file))
                <div class="mt-6 pt-6 border-t border-gray-200">
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <h6 class="text-lg font-semibold flex items-center gap-2">
                            <i class="ri-file-pdf-2-line text-red-500 text-2xl"></i>
                            {{ translate('Attached PDF') }}
                        </h6>
                        <a href="{{ asset('storage/lms/courses/topics/readings/' . $data->pdf_file) }}"
                           download
                           class="btn b-solid btn-primary-solid inline-flex items-center gap-2">
