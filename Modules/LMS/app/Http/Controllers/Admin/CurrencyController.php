...
class CurrencyController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        // 1) select: `currency` field in format "symbol-code-name"
        // 2) manual: separate `symbol`, `code`, `name` inputs (used when ext-intl is not available)
        if ($request->filled('currency')) {
            $currency = explode('-', $request->currency);
            $request->merge([
                'name' => $currency[2] ?? ($request->name ?? null),
                'code' => $currency[1] ?? ($request->code ?? null),
                'symbol' => $currency[0] ?? ($request->symbol ?? null),
