...
class CourseRepository extends BaseRepository
{
    ...
    private function handleMeetProvider($request)
    {
        if (isset($request->meet_provider_id)) {
            $meetProvider = CourseMeetProvider::updateOrCreate(
                ['course_id' => $request->course_id],
                $this->prepareMeetProviderData($request)
            );

            if ($meetProvider) {
                // Eager load user and userable status for performance
                $course = static::$model::with(['purchases.user.userable'])->find($request->course_id);
                if ($course && $course->purchases) {
                    foreach ($course->purchases as $purchase) {
                        $user = $purchase->user;
                        if ($user && $user->email) {
                            // Check if student account is deactivated (status == 0)
                            if ($user->userable && $user->userable->status == 0) {
                                continue;
                            }

                            // Check if this specific enrollment is inactive
                            if ($purchase->status === PurchaseStatus::INACTIVE) {
                                continue;
                            }

                            $data = [
                                'user_name' => $user->userable?->first_name . ' ' . $user->userable?->last_name,
                                'email' => $user->email,
                                'course_title' => $course->title,
                                'meeting_id' => $request->meeting_id,
                                'meeting_date' => $request->class_schedule_date,
                                'meeting_time' => $request->class_schedule_time,
                                'app_name' => env('APP_NAME') ?? 'EduLab',
                            ];
                            EmailFormat::liveClassScheduled($data);
                        }
                    }
                }
            }
    ...
    private function prepareSettingData($request): array
    {
        return [
            ...
            'is_subscription' => $request->is_subscription == 'on' ? 1 : 0,
        ];
    }
        ...
        $courseBundles->withWhereHas(
            ...
            function ($query) {
                $query->where('status', CourseStatus::APPROVED)
                    ...
                    }, 'courseSetting', 'coursePrice', 'reviews', 'translations']);
            }
        );
