...
class StudentRepository extends BaseRepository
{
    ...
    public static function update($id, $request): array
    {
        ...
        // Set dynamic validation rules for email and phone uniqueness
        static::$rules['update'] = array_merge(static::$rules['update'], [
        ...
        if ($request->hasFile('image')) {
            ...
            $currentImage = $user->userable->profile_img ?? '';
            $image = parent::upload($request, fieldname: 'image', file: $currentImage, folder: 'lms/students');
            $request->request->add([
                'profile_img' => $image,
            ]);
        }

        // Update User model attributes (email and password)
        ...
        // Complete the update for the Student model via parent update logic
    ...
    public function statusChange($id)
    {
        ...
        // Send notifications regarding the account status change *only* when inactivated, as per requirement.
        if ($student->status == 0 && !empty($email)) {
    ...
    public function courseEnrolled($item = null)
    {
        ...
        ])->where('status', '!=', PurchaseStatus::INACTIVE);
    ...
    public function courseEnrolledCount(): int
    {
        ...
        $purchase =  $model->where([
            'user_id' => authCheck()->id,
            'type' => PurchaseType::ENROLLED,
        ])->where('status', '!=', PurchaseStatus::INACTIVE)->count();
    ...
    public function purchaseCourses()
    {
        ...
        $query = PurchaseDetails::with('course.coursePrice', 'course.levels')
        ...
        if (module_enable_check('subscription')) {
            $query->addSelect('subscription_id');
        }
    ...
    public function bundlePurchases()
    {
        ...
        $query = PurchaseDetails::with('courseBundle.courses')
            ->where([
                'user_id' => authCheck()->id,
                'type' => PurchaseType::PURCHASE
            ])
