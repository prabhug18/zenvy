...
class EmailFormat
{
    ...
    public static function liveClassScheduled($data)
    {
        try {
            $emailContent = static::$model::where('template_name', EmailFormatStatus::LIVE_CLASS_SCHEDULED)->first();
            if ($emailContent) {
                $email_content = str_replace(
                    ['[user_name]', '[course_title]', '[meeting_id]', '[meeting_date]', '[meeting_time]', '[app_name]'],
                    [$data['user_name'], $data['course_title'], $data['meeting_id'], $data['meeting_date'], $data['meeting_time'], $data['app_name']],
